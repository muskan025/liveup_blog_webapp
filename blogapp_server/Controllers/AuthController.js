...
AuthRouter.post('/login', async (req, res) => {
  ...
  try {
    ...
    return res.send({ status: 400, message: error, error: 'Data issue' });
  }
...
AuthRouter.patch('/edit-profile/:username', async (req, res) => {
  ...
  try {
    ...
    const updatedProfile = await User.editProfile({
      userId,
      name,
      username,
      email,
...
AuthRouter.post('/logout_from_all_devices', async (req, res) => {
  ...
  try {
    const deleteSessionCount = await SessionSchema.deleteMany({
